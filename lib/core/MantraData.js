...
/*
    This object contains all the supported MantraComponent classes.
*/
const BASE_CLASSES = {
    'MantraForm': MantraForm
};
...
export default class MantraData {
    ...
    /**
     * Sets the _baseClass property.
     * @param {string} name - The base class name.
     */
    _setBaseClass(name) {
        this._baseClass = this._getBaseClass(name);
    };

    /**
     * Retrieves a MantraComponent class using the BASE_CLASSES constant.
     * @param {string} name - The base class name.
     * @returns {MantraComponent} The MantraComponent class that matches with the name parameter.
     */
    _getBaseClass(name) {
    ...
    _isCustomAction() {
        ...
        if (this.schema.actions.hasOwnProperty(action)) return true;
        else return false;
    };
    ...
    _trimPath(path) {
        ...
        this.path = this.path.replace(pathWord, '');
    };
